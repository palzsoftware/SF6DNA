...
import { buildDailyTrainingHref } from "@/lib/daily-training";
...
const AXIS_LABELS: Record<string, string> = {
  anti_air: "対空", drive_rush_defense: "ドライブラッシュ対応", impact_response: "インパクト返し", punish: "確定反撃",
  defense: "防御・切り返し", offense: "攻めの選択肢", meter: "ゲージ管理", matchup: "キャラ対策",
  execution: "コンボ・セットプレイ精度", neutral: "中距離・立ち回り", corner_defense: "画面端の守り", decision: "観察・判断",
  aggression: "前に出る攻め", patience: "待ち・観察", keepout: "遠距離・けん制", rushdown: "近距離ラッシュ",
  grappling: "投げ・コマンド投げ", setup: "設置・セットプレイ", footsies: "差し合い・地上戦", mobility: "機動力・位置調整",
  simplicity: "操作の分かりやすさ", technicality: "テクニカル操作", defense_preference: "守りから組み立てる", explosive: "一気に試合を動かす",
};
...
function buildCoachHref(diagnosisType: string, topQuery: string, issueKeys: string[]) {
  const params = new URLSearchParams({ q: topQuery, diagnosis: diagnosisType });
  const issues = issueKeys.filter((key) => IMPROVEMENT_AXES.has(key)).slice(0, 3);
...
function calculateTotals(diagnosis: DiagnosisDefinition, answers: Record<string, string>) {
  ...
  for (const q of diagnosis.questions) {
    ...
    if (!option) continue;
    for (const [key, value] of Object.entries(option.scorePayload)) totals[key] = (totals[key] ?? 0) + Number(value || 0);
  }
  return totals;
}
...
function resultCopy(type: string) {
  ...
  if (type === "character_fit") return { title: "相性が良さそうなキャラクター特性", body: "点数が高い項目ほど、キャラクター選びで重視すると相性が良くなりやすい特性です。検証済みの特性マッピングが揃ったキャラだけ推薦対象にします。", scoreLabel: "適性", searchLabel: "関連キャラ・攻略を探す" };
...
export function DiagnosisRunner({ diagnosis }: { diagnosis: DiagnosisDefinition }) {
  ...
  useEffect(() => {
    if (!hydrated) return;
    window.localStorage.setItem(draftKey(diagnosis.slug), JSON.stringify(answers));
  }, [answers, diagnosis.slug, hydrated]);

  const totals = useMemo(() => (completed ? calculateTotals(diagnosis, answers) : {}), [answers, completed, diagnosis]);
  const result = useMemo(() => Object.entries(totals).sort((a, b) => b[1] - a[1]), [totals]);

  useEffect(() => {
    if (!completed || historySaved.current) return;
    const topResults = result.filter(([, score]) => score > 0).slice(0, 6).map(([key, score]) => ({ key, label: axisLabel(key), score }));
    saveDiagnosisHistory({
      id: typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`,
      diagnosisSlug: diagnosis.slug, diagnosisTitle: diagnosis.title, diagnosisType: diagnosis.diagnosisType, topResults,
  ...
  async function loadRecommendations(scorePayload: Record<string, number>) {
    setRecommendationLoading(true); setRecommendationMessage(null);
    try {
      const response = await fetch("/api/diagnosis/recommend", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scores: scorePayload, limit: 5 }) });
  ...
  if (!diagnosis.questions.length) return <div className="empty-state"><p>公開済みの質問がまだありません。</p></div>;
  if (!hydrated) return <div className="empty-state"><p>前回の回答を確認しています…</p></div>;
  ...
  if (completed) {
    ...
    return <section className="info-panel diagnosis-result"><p className="eyebrow">RESULT</p><h2>{copy.title}</h2>{resumedCount ? <p className="data-notice">この結果には、この端末に保存されていた明示的な前回答 {resumedCount}件を再利用しています。</p> : null}<DatabaseSaveNotice status={databaseSaveStatus} message={databaseSaveMessage} onRetry={() => void saveCompletedDiagnosis()} /><p>{copy.body}</p><ResultList rows={priorities} label={copy.scoreLabel} />{diagnosis.diagnosisType === "character_fit" ? <RecommendationBlock loading={recommendationLoading} recommendations={recommendations} message={recommendationMessage} /> : null}<div className="diagnosis-actions"><Link className="button-primary" href={buildDailyTrainingHref(diagnosis.diagnosisType, result)}>今日の練習を決める</Link>{topQuery ? <Link className="button-primary" href={`/search?q=${encodeURIComponent(topQuery)}`}>{copy.searchLabel}</Link> : null}{releaseFeatures.aiCoach && topQuery ? <Link className="button-secondary" href={buildCoachHref(diagnosis.diagnosisType, topQuery, priorities.map(([key]) => key))}>AIコーチ用Evidenceを見る</Link> : null}<ResetButton onReset={reset} /></div></section>;
  }

  const progress = Math.round(((index + 1) / diagnosis.questions.length) * 100);
  const selected = answers[question.id];
  return <section className="diagnosis-runner">{resumedCount ? <div className="data-notice"><strong>前回の回答 {resumedCount}件を再利用</strong><p>この端末で以前あなたが選んだ回答だけを使い、最初の未回答質問から再開しています。推測入力は行いません。</p></div> : null}<div className="diagnosis-progress" role="progressbar" aria-label="診断の進捗" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress)}><span style={{ width: `${progress}%` }} /></div><p className="muted">{index + 1} / {diagnosis.questions.length}</p><h2>{question.prompt}</h2>{question.helpText ? <p className="muted">{question.helpText}</p> : null}<div className="diagnosis-options">{question.options.map((option) => <button className={selected === option.id ? "diagnosis-option is-selected" : "diagnosis-option"} key={option.id} type="button" onClick={() => setAnswers((current) => ({ ...current, [question.id]: option.id }))}>{option.label}</button>)}</div><div className="diagnosis-actions"><button className="button-secondary" type="button" disabled={index === 0} onClick={() => setIndex((value) => Math.max(0, value - 1))}>戻る</button><button className="button-primary" type="button" disabled={!selected} onClick={() => { const finishing = index === diagnosis.questions.length - 1; if (finishing && (diagnosis.diagnosisType === "character_fit" || diagnosis.diagnosisType === "comprehensive")) { const scorePayload = calculateTotals(diagnosis, answers); const styleScores = Object.fromEntries(Object.entries(scorePayload).filter(([key]) => !IMPROVEMENT_AXES.has(key))); void loadRecommendations(styleScores); } setIndex((value) => value + 1); }}>{index === diagnosis.questions.length - 1 ? "結果を見る" : "次へ"}</button></div></section>;
}
