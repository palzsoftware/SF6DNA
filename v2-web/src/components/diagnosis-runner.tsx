...
export function DiagnosisRunner({ diagnosis }: { diagnosis: DiagnosisDefinition }) {
  ...
  if (completed) {
    ...
    return <section className="info-panel diagnosis-result"><p className="eyebrow">RESULT</p><h2>{copy.title}</h2>{resumedCount ? <p className="data-notice">この結果には、この端末に保存されていた前回の回答 {resumedCount}件を使っています。</p> : null}<DatabaseSaveNotice status={databaseSaveStatus} message={databaseSaveMessage} onRetry={() => void saveCompletedDiagnosis()} /><p>{copy.body}</p><ResultList rows={priorities} label={copy.scoreLabel} />{diagnosis.diagnosisType === "character_fit" ? <RecommendationBlock loading={recommendationLoading} recommendations={recommendations} message={recommendationMessage} /> : null}<div className="diagnosis-actions"><Link className="button-primary" href={buildDailyTrainingHref(diagnosis.diagnosisType, result)}>今日の練習を決める</Link>{topQuery ? <Link className="button-primary" href={`/search?q=${encodeURIComponent(topQuery)}`}>{copy.searchLabel}</Link> : null}{releaseFeatures.aiCoach && topQuery ? <Link className="button-secondary" href={`/coach?q=${encodeURIComponent(topQuery)}`}>AIコーチの根拠情報を見る</Link> : null}<ResetButton onReset={reset} /></div></section>;
  }
...
function RecommendationBlock({ loading, recommendations, message }: { loading: boolean; recommendations: Recommendation[]; message: string | null }) { return <div className="data-notice"><h3>キャラクター推薦</h3>{loading ? <p>確認済みのキャラクター特性から候補を探しています…</p> : null}{!loading && recommendations.length ? <ol>{recommendations.map((item) => <li key={item.characterId}><Link href={`/characters/${item.slug}`}><strong>{item.name}</strong></Link>：一致度 {item.matchPercent}% <span className="muted">（{item.mappedTraits}/{item.activeTraits}特性を比較）</span>{item.reasons.length ? <div className="muted">主な一致: {item.reasons.map((reason) => reason.label).join(" / ")}</div> : null}</li>)}</ol> : null}{!loading && !recommendations.length ? <p>{message ?? "推薦に必要なキャラクター情報がまだ不足しています。"}</p> : null}</div>; }
