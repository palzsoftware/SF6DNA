...
export function DiagnosisRunner({ diagnosis }: { diagnosis: DiagnosisDefinition }) {
  ...
  if (completed) {
    ...
    return <section className="info-panel diagnosis-result"><p className="eyebrow">RESULT</p><h2>{copy.title}</h2>{resumedCount ? <p className="data-notice">この結果には、この端末に保存されていた明示的な前回答 {resumedCount}件を再利用しています。</p> : null}<DatabaseSaveNotice status={databaseSaveStatus} message={databaseSaveMessage} onRetry={() => void saveCompletedDiagnosis()} /><p>{copy.body}</p><ResultList rows={priorities} label={copy.scoreLabel} />{diagnosis.diagnosisType === "character_fit" ? <RecommendationBlock loading={recommendationLoading} recommendations={recommendations} message={recommendationMessage} /> : null}<div className="diagnosis-actions"><Link className="button-primary" href={buildDailyTrainingHref(diagnosis.diagnosisType, result)}>今日の練習を決める</Link>{topQuery ? <Link className="button-primary" href={`/search?q=${encodeURIComponent(topQuery)}`}>{copy.searchLabel}</Link> : null}{releaseFeatures.aiCoach && topQuery ? <Link className="button-secondary" href={`/coach?q=${encodeURIComponent(topQuery)}`}>AIコーチ用Evidenceを見る</Link> : null}<ResetButton onReset={reset} /></div></section>;
  }
