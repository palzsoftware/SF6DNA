...
export function localizeTrainingText(
  ...
  let result = String(localizeComboText(value, glossary))
    ...
    .replace(/Cラッシュ/g, "キャンセルラッシュ")
    .replace(/(^|[^A-Za-z])生DR(?=$|[^A-Za-z])/g, "$1生ドライブラッシュ")
    .replace(/(^|[^A-Za-z])DR(?=$|[^A-Za-z])/g, "$1ドライブラッシュ")
    .replace(/(^|[^A-Za-z])DI(?=$|[^A-Za-z])/g, "$1ドライブインパクト")
    .replace(/(^|[^A-Za-z])PC(?=$|[^A-Za-z])/g, "$1パニッシュカウンター")
    .replace(/(^|[^A-Za-z])CH(?=$|[^A-Za-z])/g, "$1カウンターヒット")
    .replace(/トレモ/g, "トレーニングモード")
    .replace(/確反/g, "確定反撃")
