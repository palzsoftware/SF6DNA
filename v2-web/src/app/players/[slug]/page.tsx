...
import { PlayerAnalysisPanel } from "@/components/player-analysis-panel";
...
import { isRelatedVideoPublic } from "@/lib/related-video-public-contract";
...
export default async function PlayerDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedVideos = videos.filter((video) => isRelatedVideoPublic({ status: "published", availability: video.availability, availabilityCheckedAt: video.availabilityCheckedAt, url: video.url, playerIds: video.playerIds }, player.id)).slice(0, 8);
  ...
  return <div className={`site-shell page-stack ${styles.playerPage}`}>
    ...
    <PlayerAnalysisPanel playerId={player.id} displayName={player.displayName} items={[]} />
