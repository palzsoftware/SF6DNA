...
test("strategy details expose patch, verification, and source metadata", () => {
  ...
  assert.match(source, /getPublicEntitySources/, "strategy source lookup missing");
...
test("AI coach remains retrieval-only and refuses unsourced evidence", () => {
  ...
  assert.match(source, /const ready = Boolean\(currentPatch && retrievalBundle\.evidence\.some\(\(item\) => item\.kind === "VERIFIED_GAME_FACT"\)\)/, "coach readiness requires patch and verified gameplay evidence");
  assert.match(source, /\bready,/, "computed readiness must be returned");
});

test("auth preserves a safe internal return path after login", () => {
