-- Phase14 P0-02: Public Verification Gate Hardening
-- Public strategy/frame reads must not expose reviewed/unverified data as confirmed information.
-- Admin ALL policies remain unchanged.

-- Strategy entities: public requires both publication and verification.
...
create policy "public read published setups" on public.setups
for select using (status = 'published' and verification_status = 'verified');

drop policy if exists "public read published sequences" on public.sequences;
create policy "public read published sequences" on public.sequences
for select using (status = 'published' and verification_status = 'verified');

drop policy if exists "public read published counters" on public.counters;
create policy "public read published counters" on public.counters
for select using (status = 'published' and verification_status = 'verified');

drop policy if exists "public read published trainings" on public.trainings;
create policy "public read published trainings" on public.trainings
for select using (status = 'published' and verification_status = 'verified');

-- Frame rows are public only when the parent move is public and the frame row itself is verified.
drop policy if exists "public read move frames" on public.move_frame_data;
create policy "public read move frames" on public.move_frame_data
for select using (
...
-- Keep the unified search rule explicit as defense in depth.
-- The function remains SECURITY INVOKER (default), so RLS also applies to public callers.
...
with q as (
  ...
  where x.status='published' and x.verification_status='verified' and (lower(x.name) like '%'||q.term||'%' or lower(coalesce(x.purpose,'')) like '%'||q.term||'%' or lower(coalesce(x.notation,'')) like '%'||q.term||'%')
  union all
  ...
  where x.status='published' and x.verification_status='verified' and (lower(x.name) like '%'||q.term||'%' or lower(coalesce(x.description,'')) like '%'||q.term||'%' or lower(coalesce(x.sequence_text,'')) like '%'||q.term||'%')
  union all
  ...
  where x.status='published' and x.verification_status='verified' and (lower(x.name) like '%'||q.term||'%' or lower(coalesce(x.sequence_text,'')) like '%'||q.term||'%' or lower(coalesce(x.notes,'')) like '%'||q.term||'%')
  union all
  ...
  where x.status='published' and x.verification_status='verified' and (lower(x.title) like '%'||q.term||'%' or lower(coalesce(x.summary,'')) like '%'||q.term||'%' or lower(coalesce(x.situation,'')) like '%'||q.term||'%')
  union all
  ...
  where x.status='published' and x.verification_status='verified' and (lower(x.name) like '%'||q.term||'%' or lower(coalesce(x.purpose,'')) like '%'||q.term||'%' or lower(coalesce(x.method,'')) like '%'||q.term||'%')
  union all
