...
begin
  ...
  values (
    v_user_id, v_activity_type, v_source_type, v_source_id,
    v_label, v_metadata, v_occurred_at,
...
drop trigger if exists capture_match_activity on public.user_match_logs;
create trigger capture_match_activity
after insert or update on public.user_match_logs
for each row execute function private.capture_user_activity();

drop trigger if exists capture_training_activity on public.user_training_logs;
create trigger capture_training_activity
after insert or update on public.user_training_logs
for each row execute function private.capture_user_activity();
