-- Phase23: quarantine generic matchup-verification Training templates and harden public Training publication.
-- No Training is promoted to verified/published by this migration.

update public.trainings
set status = 'archived',
    updated_at = now()
where status = 'draft'
  and verification_status = 'unverified'
  and training_type = 'matchup'
  and purpose = '30秒カード候補の主要回答を実機再現し、保証回答と読みを分離する。'
  and recording_instructions = '相手の接近/飛び/ラッシュ/主力技/端攻めを5スロット以上へ録画。'
  and method = '自キャラの対空・通常技・必殺技・パリィ・Dリバ・SAを比較し、確定/相打ち/読み/不成立で記録。'
  and next_step = '確認できた状況のみ個別Counterへ昇格する。';

create or replace function private.is_training_public_ready(target_training_id uuid)
returns boolean
language sql
stable
security definer
set search_path = public
as $$
  select exists (
    select 1
    from public.trainings tr
    join public.patches p on p.id = tr.valid_from_patch_id
    where tr.id = target_training_id
      and tr.status = 'published'
      and tr.verification_status = 'verified'
      and p.is_current = true
      and tr.valid_to_patch_id is null
      and tr.name is not null
      and btrim(tr.name) <> ''
      and tr.purpose is not null
      and btrim(tr.purpose) <> ''
      and tr.method is not null
      and btrim(tr.method) <> ''
      and tr.purpose <> '30秒カード候補の主要回答を実機再現し、保証回答と読みを分離する。'
      and tr.next_step is distinct from '確認できた状況のみ個別Counterへ昇格する。'
      and (
        tr.player_character_id is null
        or exists (
          select 1 from public.characters c
          where c.id = tr.player_character_id
            and c.status = 'published'
            and c.is_playable = true
        )
      )
      and (
        tr.dummy_character_id is null
        or exists (
          select 1 from public.characters c
          where c.id = tr.dummy_character_id
            and c.status = 'published'
            and c.is_playable = true
        )
      )
      and exists (
        select 1
        from public.entity_sources es
        where es.entity_type = 'training'
          and es.entity_id = tr.id
      )
  );
$$;

revoke all on function private.is_training_public_ready(uuid) from public;
grant execute on function private.is_training_public_ready(uuid) to anon, authenticated, service_role;

drop policy if exists "public read published trainings" on public.trainings;
drop policy if exists "public read release-ready trainings" on public.trainings;
create policy "public read release-ready trainings"
on public.trainings
for select
to anon, authenticated
using (private.is_training_public_ready(id));

create or replace function private.enforce_training_publication_ready()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if new.status <> 'published' then
    return new;
  end if;

  if new.verification_status <> 'verified' then
    raise exception 'published training requires verification_status=verified';
  end if;

  if new.name is null or btrim(new.name) = ''
     or new.purpose is null or btrim(new.purpose) = ''
     or new.method is null or btrim(new.method) = '' then
    raise exception 'published training requires name, purpose and method';
  end if;

  if new.purpose = '30秒カード候補の主要回答を実機再現し、保証回答と読みを分離する。'
     or new.next_step = '確認できた状況のみ個別Counterへ昇格する。' then
    raise exception 'generic verification-only training cannot be published';
  end if;

  if new.valid_to_patch_id is not null
     or not exists (
       select 1 from public.patches p
       where p.id = new.valid_from_patch_id
         and p.is_current = true
     ) then
    raise exception 'published training must target the current patch';
  end if;

  if new.player_character_id is not null
     and not exists (
       select 1 from public.characters c
       where c.id = new.player_character_id
         and c.status = 'published'
         and c.is_playable = true
     ) then
    raise exception 'published training player must be a published playable character';
  end if;

  if new.dummy_character_id is not null
     and not exists (
       select 1 from public.characters c
       where c.id = new.dummy_character_id
         and c.status = 'published'
         and c.is_playable = true
     ) then
    raise exception 'published training dummy must be a published playable character';
  end if;

  if not exists (
    select 1 from public.entity_sources es
    where es.entity_type = 'training'
      and es.entity_id = new.id
  ) then
    raise exception 'published training requires Source evidence';
  end if;

  return new;
end;
$$;

drop trigger if exists enforce_training_publication_ready on public.trainings;
create trigger enforce_training_publication_ready
before insert or update of status, verification_status, name, purpose, method, next_step, valid_from_patch_id, valid_to_patch_id, player_character_id, dummy_character_id
on public.trainings
for each row
execute function private.enforce_training_publication_ready();
