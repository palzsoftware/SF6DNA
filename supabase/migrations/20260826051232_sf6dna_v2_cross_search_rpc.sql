...
with q as (
  ...
  select 'character'::text entity_type, c.id entity_id, c.slug, c.name_ja title, c.summary subtitle,
    'name'::text matched_by,
    greatest(similarity(lower(c.name_ja), q.term), case when lower(c.name_ja)=q.term then 1 else 0 end)::real score
  from public.characters c cross join q
  where c.status='published' and (lower(c.name_ja) like '%'||q.term||'%' or lower(c.name_en) like '%'||q.term||'%' or similarity(lower(c.name_ja), q.term) > 0.2)

  union all
  select 'character', c.id, c.slug, c.name_ja, '別名: '||a.alias, 'alias',
    greatest(similarity(a.normalized_alias, q.term), case when a.normalized_alias=q.term then 1 else 0 end)::real
  ...
  union all
  select 'move', m.id, m.slug, m.name_ja, c.name_ja, 'name',
    greatest(similarity(lower(m.name_ja), q.term), case when lower(m.name_ja)=q.term then 1 else 0 end)::real
  ...
  union all
  select 'move', m.id, m.slug, m.name_ja, '別名: '||a.alias||' / '||c.name_ja, 'alias',
    greatest(similarity(a.normalized_alias,q.term), case when a.normalized_alias=q.term then 1 else 0 end)::real
  ...
  union all
  select 'combo', x.id, x.slug, x.name, c.name_ja, 'name', similarity(lower(x.name),q.term)::real
  from public.combos x join public.characters c on c.id=x.character_id cross join q
  where x.status='published' and (lower(x.name) like '%'||q.term||'%' or lower(coalesce(x.purpose,'')) like '%'||q.term||'%')
  ...
  union all
  select 'player', p.id, p.slug, p.display_name, coalesce(p.team_name,p.player_type), 'name',
    greatest(similarity(lower(p.display_name),q.term), case when lower(p.display_name)=q.term then 1 else 0 end)::real
  ...
  union all
  select 'player', p.id, p.slug, p.display_name, '別名: '||a.alias, 'alias',
    greatest(similarity(a.normalized_alias,q.term), case when a.normalized_alias=q.term then 1 else 0 end)::real
  ...
  union all
  select 'glossary', g.id, g.slug, g.term, g.short_definition, 'name',
    greatest(similarity(lower(g.term),q.term), case when lower(g.term)=q.term then 1 else 0 end)::real
  ...
  union all
  select 'glossary', g.id, g.slug, g.term, '別名: '||a.alias, 'alias',
    greatest(similarity(a.normalized_alias,q.term), case when a.normalized_alias=q.term then 1 else 0 end)::real
  ...
  select *, row_number() over (partition by entity_type, entity_id order by score desc, matched_by) rn
  from candidates
)
...
grant execute on function public.search_sf6dna(text, integer) to anon, authenticated;
