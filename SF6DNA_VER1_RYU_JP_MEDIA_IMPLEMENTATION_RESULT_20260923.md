...
- Boundary: static pilot media is returned only in `VERCEL_ENV=preview`; the protected Preview can be visually checked without exposing the device-preview token. RPC draft data still requires the token. Public/Production published-media behavior is unchanged.
